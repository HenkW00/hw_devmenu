-- Clear Inventory
RegisterNetEvent('hw_devmenumenu:server:ClearInventory', function(data, selectedData)
...
-- Open Inv [ox side]
RegisterNetEvent('hw_devmenumenu:server:OpenInv', function(data)
...
-- Give Item to All
RegisterNetEvent('hw_devmenumenu:server:GiveItemAll', function(data, selectedData)
    if not CheckPerms(data.perms) then return end
    local src = source
    local item = selectedData["Item"].value
    local amount = selectedData["Amount"].value
