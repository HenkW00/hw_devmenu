-- Admin Car
RegisterNetEvent('hw_devmenumenu:server:SaveCar', function(mods, vehicle, _, plate)
    local src = source
...
-- Change Plate
RegisterNetEvent('hw_devmenumenu:server:ChangePlate', function(newPlate, currentPlate)
...
lib.callback.register('hw_devmenumenu:getVehicleData', function(source, plate)
    local vehData = {}
    if Config.Framework == "QBCore" then
...
lib.callback.register('hw_devmenumenu:spawnVehicle', function(source, model, coords, warp)
...
-- lib.callback.register('hw_devmenumenu:server:GetVehicleByPlate', function(source, plate)
--     local result = {}
...
RegisterNetEvent('hw_devmenumenu:server:FixVehFor', function(data, selectedData)
